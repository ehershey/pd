...
string query_exit_display(string how) {
  ...
  switch (sizeof(exs)) {
    ...
    case 1:
      if (how == "short")
        msg = " "+query_short_direction(exs[0])+" ";
      else
    ...
    default:
      exs = sort_array(exs,
        function(string a, string b, string *primaries) {
          int apos = member_array(a, primaries);
          int bpos = member_array(b, primaries);
          if (apos == -1 && bpos == -1)
            return strcmp(a, b);
      ...
      if (how == "short") {
        ...
        msg = replace_string(list(exs, 0, 0), ",", " %^CYAN%^:%^RESET%^");
      }
      else
  ...
  switch (sizeof(ents)) {
    ...
    case 1:
      entermsg += "\nThere is one entrance: "+ents[0][sizeof(ENTER_PREFIX)..<1];
    break;

    default:
      ents = sort_array(ents, 1);
      entermsg += "\nThere are "+cardinal(sizeof(ents))+" entrances: "+list(ents)+".";
  }
...
void use_exit(object tp, string dir) {
  ...
  ex = find_exit_name(dir) || find_exit_alias(dir);
  if (!ex) {
    message("info", sprintf(BAD_DEST_MSG, "dir"), tp);
    return;
  }
  ...
  // function should return 1 to block, 0 to allow
