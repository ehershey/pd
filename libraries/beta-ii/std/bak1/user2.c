...
#define ALL_GUILDS ({ "darksoul", "unholy", "blackforce", "forsaken", "illuminati", "chosen", "AoD"})
#define ALL_LAW ({ "chromium" })
#define HMA_LINE ({ "wiz", "faust", "wildflower", "staver", "yakko", "kalohe", "ying",\
      "akamai", "baitzi", "sulphur", "taz", "seeker", "stormbringer", "nightshade" })
...
string query_guild_position() {
   if(!query_guild()) return "None";
    return GUILD_D->query_position(query_name(), query_guild());
}
//  end add by Whit
...
int quit(string str) {
 string msg;    
    ...
    if (this_object()->query_current_attacker()) {
        notify_fail("And flee battle?  You coward!\n");
        return 0;
    }
...
void setup() {
    ...
    this_object()->remove_quest("tenebrae");
    this_object()->remove_quest("Kings Quest");
    set_peace_area(1);
...
// if (!this_object()->query_class()) this_object()->init_limb_data();
...
nomask void die() {
    ...
    if(this_object()->query_mounted())
      this_object()->query_mounted()->force_me("dismount "+this_object()->query_name());
   ...
   tmp = 0;         
    ...
    if(this_object()->query_current_attacker() ? this_object()->query_current_attacker()->query_race() != "demon" : 0)
...
void register_channels() {
    ...
    if (query_name()=="wiz" && !wizardp(this_object())) channels += ({ "driver" });
    if (member_array(query_name(), ALL_LAW) != -1 || wizardp(this_player()))
       channels += ({ "reg" });
    if (member_array(query_name(), HMA_LINE) != -1 || query_ip() == "64.65.64.17")
       channels += ({ "hma" });
    ...
    channels += ({ "colleseum", "gossip", "OOC", "trivia" });
...
void sync_msql(string name, int i)
{
 ...
 err= MSQL_D->msql_update("mud", "player_t", SET, "player_name='"+name+"'");
 } else {
 ...
 err =  MSQL_D->msql_insert("mud", "player_t", set);
  }
}
if (err) write(err);
}
