...
static int t_time;
static int Disable, DisableStart, MagicRound, MagicRoundStart;
static mapping Allow;
static object *CurrentPets;
static mixed *last_death_source;
...
int query_busy();
int query_disable();
int query_disable_start();
void set_disable(int x);
int query_magic_round();
int query_magic_round_start();
void set_magic_round(int x);
varargs string *make_corpse(object);
mixed *find_death_source(mixed source);
...
void create() {
  ...
  Allow = ([]);
  CurrentPets = ({});
  Disable = MagicRound = 0;
  DisableStart = MagicRoundStart = 1;
}
...
void heart_beat() {
  ...
  if (to->query_hp() < 1 && !to->query_ghost() &&
      ...
      !(to->query_property("vampire death")) )
...
int query_disable_start() { return DisableStart; }
void set_disable(int x) { if (nullp(x)) x = 1; Disable = DisableStart = x; if (DisableStart < 1) DisableStart = 1; }
int query_magic_round() { return MagicRound; }
int query_magic_round_start() { return MagicRoundStart; }
void set_magic_round(int x) { if (nullp(x)) x = 1; MagicRound = MagicRoundStart = x; if (MagicRoundStart < 1) MagicRoundStart = 1; }
...
varargs string *make_corpse(object room) {
  object to = this_object();
  object corpse, money;
  object *items;
  string *ret = ({});
  ...
  all_inventory()->silent_unequip();

  // move all items into corpse that don't have 'keep death' property
  items = filter(all_inventory(), (: !$1->query_property("keep death") :));
  if (sizeof(items)) {
    items->move(corpse);
    ret += map(items, (: base_name($1) :) );
  }

  // anything that wouldn't move goes into the room
  items = filter(items, (: environment($1) == $2 :), to);
  if (sizeof(items))
    items->move(environment());

  // if it still wouldn't move, destroy it
  items = filter(items, (: environment($1) == $2 :), to);
  if (sizeof(items))
...
mixed *find_death_source(mixed source) {
  ...
  if (last_death_source)
    return last_death_source;

  room = environment(to);

  if (nullp(source) || !source) {
    source = to->query_current_attacker();
    if (!source) {
      if ( (time() - to->query_last_damage_time()) <= 2) {
	source = to->query_last_damage();
      }
      zone = to->query_last_damage_zone();
    }
  }
  ...
  if (objectp(source)) {
    if (source->is_living()) {
      ...
      desc = source->query_short() || source->query_name();
    }
  }

  if (!zone) {
    zone = room->query_zone() || "pk";
  }

  if (!desc) {
    if (killer) {
      desc = remove_article(strip_color(killer->query_name()));
    }
    else {
      desc = "spontaneous combustion";
    }
  }
  else {
  ...
  last_death_source = ({ killer, desc, room, zone });
  return last_death_source;
}
...
varargs void die(mixed source) {
  ...
  if (to->is_player() && to->query_arena()) return;

  source = find_death_source(source);
  killer = source[0];
  desc = source[1];
  room = source[2];
  ...
  if (sizeof(all_inventory()))
    all_inventory()->notify_die(source);
      
  if (to->is_monster() &&
      !to->query_property("no corpse") && !to->query_property("no_corpse")) {
	make_corpse();
  }

  // even if a mob is made to level up, it shouldn't get bonus exp
  // so keep this outside the next block
  bonus = ADVANCE_D->query_exp_bonus();

  // these things are done here in case mobs are ever made to care about them
  // examples: tracking who they've killed, leveling up, etc...
  if (killer && living(killer)) {

    killer->remove_attacker(to);
    killer->remove_hunter(to);
    to->remove_attacker(killer);
    to->remove_hunter(killer);

    killer->adj_alignment( to->query_alignment() );
    killer->add_kill( to->query_name() );

    exp = ADVANCE_D->query_exp_for_kill(killer, to);

    killmsg = "You feel more experienced for the kill. (";
    if (environment(killer) && room &&
	environment(killer) != room)
	  killmsg += "%^CYAN%^"+to->query_cap_name()+"%^RESET%^ ";

    killmsg += "%^BOLD%^"+add_commas(exp)+"%^RESET%^";
    if (bonus > 100)
      killmsg += " %^YELLOW%^bonus: +"+(bonus-100)+"%";
    if (bonus < 100)
      killmsg += " %^RED%^scourge: -"+(100-bonus)+"%";
    killmsg += "%^RESET%^)";

    message("info", killmsg, killer);
...
int move(mixed dest) {
    ...
    if (!prev->query_property("no zone") && !env->query_property("no zone") &&
        file_name(prev) != ROOM_VOID && file_name(env) != ROOM_VOID &&
        env->query_zone() != prev->query_zone())
          env->show_zone(to);
...
void add_alignment(int x) {
    ...
    if (race) {
      string mod = RACE_D->query_alignment(race);
      if (mod == "good" && x < 0) x /= 2;
      if (mod == "evil" && x > 0) x /= 2;
    }
    if(x>40) x = 40;
