...
int balance(string str) {
    ...
    "%^RESET%^%^BLUE%^=+=+=+=+=+=+=+=+=+=+=+=+=+=+=%^RESET%^\n";
    */
    out = header_line(capitalize(guild)) + "\n";
    while(i--) {
...
int withdraw(string str) {
    ...
    // only imms allowed to withdraw
    if (this_player() && !wizardp(this_player()))
      return notify_fail("Only immortals may withdraw from the treasury.\n");
    if(!str) {
    ...
    if(this_player()->query_original_guild() != query_guild() &&
      !wizardp(this_player())) {
...
int read(string str) {
    ...
    if(str == "summary") {
      this_player()->more( explode(TREASURY_D->read_summary(guild), "\n") );
      return 1;
    }
}
