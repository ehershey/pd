...
varargs string fixId(string, string);
...
void set_locked(string id, int locked) {
  string newid = fixId(id);
  string use;

  if (is_door(newid)) use = newid;
  else if (is_door(id)) use = id;
  else return;

  DOOR_D->set_locked(use, locked);
}
...
varargs string fixId(string oldId, string dir) {
  ...
  if (sscanf(oldId, "%*s %*d") == 2) oldId = replace_string(oldId, " ", "");
  if (nullp(dir)) {
    string *tmp;
  ...
  if (file2[<2..<1] == ".c") file2 = file2[0..<3];
...
string query_door(string dir) {
  string id;
  string *allids;
  if (dir) allids = get_doors_blocking_exit(dir);
  if (sizeof(allids)) id = breakId(allids[0]);
  return id;
}
...
int pick_lock(string id) { id = fixId(id); set_locked(id, 0); }
