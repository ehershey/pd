...
int valid_type(object ob)
{
	...
	if (__Type[n]=="skin"  && (sizeof(ob->query_id()) && ob->query_id()[0] == "skin"))  return 1;
	if (__Type[n]=="object" &&
	  !ob->is_weapon() && !ob->is_armour() &&
	  !ob->is_light() &&  (sizeof(ob->query_id()) && ob->query_id()[0] != "skin")) return 1;
    }
    return 0;
}
...
static int __CheckDiscrimination() {
    ...
    if(__Discriminate["race"] && member_array((string)this_player()->query_race(), __Discriminate["race"]) == -1) {
	this_object()->force_me("say I do not do business with people like you.");
	return 0;
    }
