...
//    if (file_name(env) == ROOM_FREEZER) {
  ...
  if (!to->query_rest_type() && to->query_property("teleport follow")) {
    if (env != owner_env && member_array(to, (object *)owner->query_followers()) != -1 && to->query_following() == owner) {
      if (!owner_env || owner_env->query_property("sky")) {
        owner = 0;
        return;
      }
      else {
        to->move_player(owner_env, "following "+owner->query_cap_name());
      }
    }
  }
