...
void set_disable(int x) { if (nullp(x)) x = 1; Disable = DisableStart = x; }
int query_magic_round() { return MagicRound; }
int query_magic_round_start() { return MagicRoundStart; }
void set_magic_round(int x) { if (nullp(x)) x = 1; MagicRound = MagicRoundStart = x; }
...
varargs void die(object killer) {
  ...
  if (killer && living(killer)) {
    ...
    if (environment(killer) && environment(to) && environment(killer) == environment(to))
      message("info", "You feel more experienced for the kill. "
       "(%^BOLD%^"+add_commas(exp)+"%^RESET%^)", killer);
    else
      message("info", "You feel more experienced for the kill. "
       "("+to->query_name()+" %^BOLD%^"+add_commas(exp)+"%^RESET%^)", killer);
    killer->add_exp(exp);
