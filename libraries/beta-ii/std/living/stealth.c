...
#define STEAL_TIME 60*7 // 7 minutes
...
static string *NonDisguisedActions = ({
  ...
  "demote", "promote", "induct", "reject", "resign", "change",
});
...
int query_stolen(object tgt) {
  ...
  if (a == -1) return 0;
  if (StoleFrom[2][a] < time() - STEAL_TIME) {
    remove_stolen(tgt);
    return 0;
  }
...
void remove_stolen(object tgt) {
  ...
  StoleFrom[2] = StoleFrom[2][0..a-1] + StoleFrom[2][a+1..<1];
}
void add_stolen(object tgt, int x) {
  ...
  if (a == -1) {
    ...
    StoleFrom[1][a] += x;
    if (StoleFrom[1][a] < 1) remove_stolen(tgt);
    else StoleFrom[2][a] = time();
  }
}
