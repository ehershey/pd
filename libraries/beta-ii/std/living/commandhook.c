...
nomask static int cmd_hook(string cmd) {
  ...
  string file, exitname;
  int flag = 0;
  string *gagged_commands = ({
    "say", "speak", "yell",
    "shout", "emote", "whisper",
  ...
  if (verb != "quit" && tp->query_paralyzed() && !archp(tp)) {
    message("my_action", tp->query_paralyze_message(), tp);
    return 1;
  }

  if (!verb || verb == "") return 1;

  exitname = verb;
  if (strlen(cmd))
    exitname += " "+cmd;
  if (env && strlen(exitname) && env->query_exit(exitname) != ROOM_VOID) {
    env->use_exit(tp, exitname);
    return 1;
  }
  ...
  if (tp->query_property("gagged") || tp->query_property("torture")) {
    ...
    if ((userp(tp) && member_array(stripped, tp->query_channels()) != -1 && cmd) ||
        ...
        member_array(verb, gagged_commands) != -1 ||
