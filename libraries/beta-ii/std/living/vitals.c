...
void add_mp(int mp);
int query_mp();
varargs int fix_vital_bonus(int i);
...
void do_status() {
  ...
  if (query_stuffed() > query_max_stuffed()) set_stuffed(query_max_stuffed());
  if (query_quenched() > query_max_quenched()) set_quenched(query_max_quenched());
  if (query_intox() > query_max_intox()) set_intox(query_max_intox());
...
void do_healing() {
  ...
  if (!vamp) {
    ...
    limbs += rate + all + random(4);
    limbs += (int)query_heal_rate_bonus("hp")/2;
  }
  ...
  // this is to fix vamp mobs slowly dying on their own
  if (vamp && !to->is_player()) h = 0;

  if (to->query_poisoned()) {
    if (h > 3) h = 2+random(3);
    if (s > 0) s /= 3;
    if (m > 0) m /= 2;
    if (limbs > 0) limbs /= 3;
  }
  else if (to->query_bleeding()) {
    if (h > 0) h /= 2;
    if (s > 0) s = s*3/4;
    if (limbs > 0) limbs = limbs*3/4;
  }
...
int set_heal_rate(int x) { return Vitals["heal rate"] = to_int(x); }
int query_heal_rate() { return Vitals["heal rate"]; }

void set_heal_rate_bonus(string s, int b) {
  if (s && (s == "hp" || s == "sp" || s == "mp" || s == "all"))
    VitalsTemp[s+" heal bonus"] = to_int(b);
}
void add_heal_rate_bonus(string s, int b) {
...
void adjust_biorhythms() {
  ...
  temps = to_float(to->query_age()/1000);

  freq = to_float(to->query_stats("wisdom"));
  Vitals["spiritual"] = to_int(5 * sin(freq*temps) );
  freq = to_float(to->query_stats("strength"));
  Vitals["physical"] = to_int(5 * sin(freq*temps) );
}

// these range from -5 to +5
...
void set_max_hp(int hp) { Vitals["max hp"] = to_int(hp); }
int query_max_hp() { return Vitals["max hp"]; }

void set_hp(int hp) {
  if (!query_max_hp()) set_max_hp(hp);
  if (hp > query_max_hp()) hp = query_max_hp();
  // a hack for bane spell
  if (hp > query_hp() && this_object()->query_effect("bane"))
    return;
  hp = this_object()->resolve_hooks("set_hp", hp);
  Vitals["hp"] = to_int(hp);
}
void add_hp(int hp) {
  hp = this_object()->resolve_hooks("add_hp", hp);
  set_hp(query_hp()+hp);
}
int query_hp() { return Vitals["hp"]; }

void set_max_sp(int sp) { Vitals["max sp"] = to_int(sp); }
...
void set_sp(int sp) {
  ...
  if (sp < -200) sp = -200;
  sp = this_object()->resolve_hooks("set_sp", sp);
  Vitals["sp"] = to_int(sp);
}
void add_sp(int sp) {
  int v, y;
  ...
  sp = this_object()->resolve_hooks("add_sp", sp);
  set_sp(query_sp()+sp);
}
int query_sp() { return Vitals["sp"]; }

void set_max_mp(int mp) { Vitals["max mp"] = to_int(mp); }
...
void set_mp(int mp) {
  ...
  if (mp < -200) mp = -200;
  mp = this_object()->resolve_hooks("set_mp", mp);
  Vitals["mp"] = to_int(mp);
}
void add_mp(int mp) {
  mp = this_object()->resolve_hooks("add_mp", mp);
  set_mp(query_mp()+mp);
}
...
void set_vital_bonus() {
    ...
    string cl;
    to = this_object();
    if (to->is_monster()) return; // mob vitals are set via adjust_vitals() in monster.c
    cl = to->query_class();
    /* hp */
    switch (cl) {
