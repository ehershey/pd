...
#define SEND_DELAY_SECS 40
#define SEND_MAIL_PROP "mail_send_time"
...
void get_to(string str) {
    if ((int)this_player()->query(SEND_MAIL_PROP)+SEND_DELAY_SECS > time()) {
      message("mail", "Mailing limited to 1 message every "+SEND_DELAY_SECS+" seconds.", this_player());
      return;
    }
    if(!str || str == "") {
...
void get_cc(string str) {
    ...
    this_player()->set(SEND_MAIL_PROP, time());
    if(sizeof(who)) 
