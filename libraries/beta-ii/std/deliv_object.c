// File:      /std/deliv_object.c
// Purpose:   To make a Deliverable Object for the Delivery System
// Mudlib:    Nightmare
// Credits:
/*  Original 'object.c' credits. */
...
/* Deliverable Object credits. */
// 02-03-01   Drackmire made the object deliverable
/* Begin. */
// New Features: The startclockdevob command.  It will start the clock..
// The devobtimestart() function, used for the starting time.
// The devobtimeperiod() function, used to set the time limit before it
// gets delivered.
...
void   set_devobtimestart(int x)    { set("timestart", x); }
void   set_devobtimeperiod(int x)   { set("timeperiod", x); }
void   set_dev_value(int x)      { set("dev_value", x); }
void   set_dev_exp(int x)        { set("dev_exp", x); }
...
int    query_devtimestart()      { return (int)this_object()->query("timestart");}
int    query_dev_value()         { return (int)this_object()->query("dev_value");}
int    query_dev_exp()           { return (int)this_object()->query("dev_exp");}
int    query_devtimeperiod()     { return (int)this_object()->query("timeperiod");}
...
int resetclock() {
  if(environment(this_object())->query_name()!="Headmaster") return 0;
    this_object()->set_devobtimestart(time());
    return 1;
    }
...
void init() {
    ...
    add_action("resetclock", "resetclockdevob");
 }
int move(mixed dest) {
    this_object()->unequip();
    return ::move(dest);
