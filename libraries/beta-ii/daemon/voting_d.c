...
/*    if (file_size(DIR_VOTES+"/"+ELECTIONS[0]) < 0)
      save_object(DIR_VOTES+"/"+ELECTIONS[0]);
     else*/
...
int register_vote(string who, string whom, string p_class) {
    if(!elir) return NOT_ELECTION_DAY;
    if(!who || !whom || !p_class) return VOTE_ERROR;
    if(member_array(p_class, ELECTIONS) == -1) return VOTE_ERROR;
    if(current_class != p_class) {
...
int register_candidate(string who, string p_class) {
    if(!elir) return NOT_ELECTION_DAY;
    if(!who || !p_class) return VOTE_ERROR;
    if(member_array(p_class, ELECTIONS) == -1) return VOTE_ERROR;
    if(current_class != p_class) {
	current_class = p_class;
        seteuid(UID_VOTESAVE);
...
string *query_candidates(string p_class) {
    if(!elir) return ({});
    if(member_array(p_class, ELECTIONS) == -1) return ({});
    if(current_class != p_class) {
	current_class = p_class;
        seteuid(UID_VOTESAVE);
	restore_object(DIR_VOTES+"/"+current_class);
