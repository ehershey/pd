...
*/
        ...
        if(stringp(tmp9=(string)ob->query_guild()) && tmp9 != "assassin")
           finger += "%^MAGENTA%^Guild:%^RESET%^ "+pluralize(tmp9)+"\n";
        ...
        finger += align_str("%^MAGENTA%^Birthday:%^RESET%^ "+capitalize(month(birth))+" "+date(birth)+", "+
           ((year(birth)-18))+" BN", 41);
        finger += "%^BOLD%^%^CYAN%^Age:%^RESET%^ <test>\n";
        if(this_player() && archp(this_player()))
                if((uh_oh = (string)ob->query_email()) != "")
                    finger += "%^MAGENTA%^Email:%^RESET%^ " + uh_oh + "\n";     
        if(ob->query_invis()) 
            finger += "%^BOLD%^%^WHITE%^Last on:%^RESET%^ ";
        else {
            uh_oh = query_hah(ob, 1);
            if(uh_oh != "") 
                finger += uh_oh + "\n";
            finger += "%^BOLD%^%^WHITE%^On since:%^RESET%^ ";
        }
...
/*      if(ob->query_property("where block") ) 
          ...
          finger += (string)ob->query_ip() + "\n";
*/
        finger += "%^BOLD%^%^WHITE%^" + query_cap_name() + " has ";
        if(!mail_stat["unread"]) 
        ...
        if(file_size(tmp+".plan") >= 0) 
           finger += "%^BOLD%^%^BLUE%^Plan:%^RESET%^\n"+read_file(tmp+".plan");
            ...
            finger += "%^BOLD%^%^BLUE%^Plan: %^RESET%^To forge a life out of this meager existance.%^RESET%^\n";
    }
    else {
            complete_variables();
            finger = "\n%^RESET%^%^CYAN%^[%^WHITE%^*%^CYAN%^]%^BLUE%^------%^BOLD%^(%^WHITE%^Finger%^BLUE%^)%^RESET%^%^BLUE%^--------------------------------------------------%^CYAN%^[%^WHITE%^*%^CYAN%^]%^RESET%^\n";
