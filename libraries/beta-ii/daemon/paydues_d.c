...
int cmd_pay(string str) {
    ...
    if (NO_DUES)
      return notify_fail("Dues are currently not being collected.\n");
    ...
    {
        amount = DUES_AMOUNT * sizeof(GUILD_D->query_players(guild));
	error = TREASURY_D->withdraw(this_player()->query_name(), guild,
	  amount, "gold");
	switch( error )
	{
	case INSF:
	    return notify_fail("You need at least " + amount
