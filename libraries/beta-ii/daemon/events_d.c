...
varargs void add_event(object ob, string fun, int when, mixed *args, int reg) {
  add_events(ob, fun, when, args, reg);
}
...
void update_who_html() {
    string msg;

  msg = "/cmds/mortal/_who"->list_users(({}), "l");
  rm("/tmp/who.html");
  /*write_file("/tmp/who.html", TERMINAL_D->html_it(msg));     */
  return;
}
void change_sky() {
    string msg;
...
int query_day_of_month(int x) { return CURRENT_DOM(x)+1; }
