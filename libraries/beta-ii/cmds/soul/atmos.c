...
describe_action( string verb, object victim, string pre, string post, int flags )
{
	...
	LAST_D->add_say(this_player()->query_name(), msg);
	/* tell the commandee */
	if( victim )
	{
		...
		if( flags & VARIABLE ) msg = expand_macro( msg, flags, 0 );
	      ...
	      LAST_D->add_say(victim->query_name(), msg);
	}
	else
		victim = this_object();
	...
	message("emote", msg, environment(this_player()),
	  ({ this_player(), victim }));
	all_vics = all_inventory(environment(this_player()));
	x = sizeof(all_vics);
	while (x--) {
           r = x-1;
           if (!all_vics[x]->is_player()) continue;
           if (all_vics[x] == victim) continue;
	   if (all_vics[x] == this_player()) continue;
	   vics += ({ all_vics[x] });
        }
	x = sizeof(vics);
        while (x--) {
           LAST_D->add_say(vics[x]->query_name(), msg);
	}
	return 1;
}
...
// DEBUG PETER
	...
	if( ! ( argv = extract_args( args ) ) ) return 0;
        ...
        if( present("gagger1234", this_player()) ) return 0;
...
void heart_beat()
{
	...
	dat = explode(read_file(SOUL_DB, line, 10), "\n");
...
// DEBUG PETER
#if 1
		if( ! expl || sizeof( expl ) < 6 ) {write(dat[i]+"\n"); continue;}
#else
