...
#define DPATHS     ({ MAIN+"general/", MAIN+"rules/", MAIN+"user/", MAIN+"skills/", MAIN+"commands/", MAIN+"guilds/" })
#define SUBS	   "/doc/help/classes/"
...
int cmd_help(string s) {
  ...
  if(s && s != "") {
    ...
    if (s == "race" || s == "races") {
      ...
      races = map(races, (: "  "+capitalize($1) :) );
      file = format_page(races, 5);
      message("help", header_line("Races of Primal Darkness") + "\n" +
        file + header_line(), this_player() );
      return 1;
    }
    if (RACE_D->is_player_race(s)) {
      message("help", RACE_D->query_help_file(s), this_player());
      return 1;
    }
	...
	i=sizeof(get_dir(path));
	while(i--) 
	  if(file_exists(path+get_dir(path)[i]+"/"+s)) {
	    file=path+get_dir(path)[i]+"/"+s;
	    cat=path+get_dir(path)[i];
	  }
      }
    }
    if(!file) {
