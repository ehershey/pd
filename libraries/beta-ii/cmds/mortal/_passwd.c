...
/*    if(name == "guest") {
    ...
    write("Changing password for "+name+" on Primal Darkness");
    message("no_wrap", "Old password:", this_player());
    input_to("oldpass", 1);
    return 1;
}
...
nomask static int oldpass(string pass) {
    ...
    seteuid(getuid());
    if (password != oldcrypt(pass, password)&&
        password != crypt(pass, password)) return 0;
