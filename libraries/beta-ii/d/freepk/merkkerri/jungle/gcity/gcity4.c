...
set_short("%^YELLOW%^The Quixxiol Hospital%^RESET%^");
set_long("%^YELLOW%^You are in the local hospital run by the town's clerics.  %^YELLOW%^Here they specialize in the regeneration of lost limbs.  A %^YELLOW%^list of all services they perform is posted on the wall.%^RESET%^");
    set_items(
    ...
    set_exits( 
(["north" : GCITY "gcity2.c"]) );
    blood = ([ "who": ([]), "hp":500, "mp":500 ]);
...
int new_body(string str) {
    ...
    say("A cleric mutters a small prayer for the novice
"+this_player()->query_cap_name()+".", this_player());
...
int read(string str) {
    ...
    message("info", "Welcome to the Cleric's Hospital of
Quixxiol.",this_player());
    message("Ninfo",
...
*/
    ...
    limb_info=
(mapping)RACE_D->query_limb_info(limb,(string)tp->query("race"));
    if(!limb_info) {
    ...
    if(limb_info["attach"] != "0") {
        if(member_array(limb_info["attach"], there) == -1) {
            notify_fail("You would need a "+limb_info["attach"]+" for
that!\n");
         return 0;
     }
    }
