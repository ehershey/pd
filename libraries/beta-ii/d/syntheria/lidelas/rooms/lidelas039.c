...
void create()
{
    ::create();
        ...
        "gargoyles"    :  "They are not very large, and seem to be carved into the building."
            ...
            set_exits(([
                        "north"         : ROOMS "lidelas027.c",
                        "south"         : ROOMS "lidelas001.c",
        ]));
}
...
void reset() {
    object *childs;
    ::reset();
        childs = children(MOB"eadfrid_tondhere.c");
        if(find_object(MOB"eadfrid_tondhere.c")) childs -= ({find_object(MOB"eadfrid_tondhere.c") });
        if (childs==({}) || !sizeof(childs))
            new(MOB "eadfrid_tondhere.c")->move(this_object());
}
