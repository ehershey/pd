...
set_long("The molten tunnels ruthlessly %^YELLOW%^%^BOLD%^boil%^RESET%^ and %^RED%^%^BOLD%^burn%^RESET%^ beneath the rocky path. %^BOLD%^%^WHITE%^Bones%^RESET%^ float throughout the %^RED%^l%^BOLD%^a%^ORANGE%^%^RESET%^v%^YELLOW%^a%^BOLD%^%^RESET%^... what sort of being would be so strong to have indestructible bones? Shadows seem to creep through the smokey haze, although nothing can be seen for sure. Wisps of smoke float through the air, sometimes making shapes. All of the air seems to vibrate in the waves of heat.");
set_items(([
...
set_exits(([ "west" : ROOMS "/moltenlava/35.c"]));
}
