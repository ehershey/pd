...
void create() {
    ::create();
    set_properties((["indoors":1, "no scry":1, "light":1]));
    set_short("Illuminati Tower");
    set_long("%^BOLD%^WHITE%^The Tirun Bank, Illuminati Branch.\n%^RESET%^ORANGE%^After a generous donation by the Illuminati, the bank of Tirun has set up a new branch within the tower of light, strictly for use by members of the Illuminati. There is a sign you can read.%^RESET%^");
    set_exits(([
		"down": ILROOM"treasury",
		]));
}
...
void reset() {
    ::reset();
    if(!present("guard"))
      new(ILMOB"bankguard.c")->move(this_object());
}
...
int open(string str) {
    ...
    write("You open up an account at the Tirun Merchants Bank, Illuminati Branch.");
...
int balance(string str) {
    ...
    out = "\n%^BLUE%^=+=+=+=+=+=+=+=+%^BOLD%^("
    "%^WHITE%^Bank of Tirun - Illuminati Branch%^BLUE%^)"
    "%^RESET%^BLUE%^=+=+=+=+=+=+=+=+=+=+=%^RESET%^\n";
    while(i--) {
...
int read(string str) {
    ...
    message("info",
      "You may do any of the following at Tirun Merchants Bank, Illuminati Branch:\n"
