...
void create() {
   ::create();
     set_properties((["light": -1, "night light" : -2, "indoors" : 1]));
...
void init() {
   ::init();
     add_action("leave", "up");
     if(this_player() && this_player()->is_player() && !present("worthmark", this_player()) && !present("demon", this_object()) && member_array("kratoan", this_player()->query_quests()) == -1)
...
int leave() {
    ...
    env = find_object("/d/shadow/rooms/tenehouse.c");
    if(env)
