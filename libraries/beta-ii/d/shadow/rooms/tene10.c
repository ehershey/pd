...
void create() {
    object board;
    room::create();
    set_properties( ([ "light" : 1, "indoors" : 0, "no attack" : 1, "no magic" : 1, "no bump" : 1]) );
    set_short("Tenebrae Square");
    set_long( "This is Tenebrae Square. People hurry about their business to the north and south. The many shops are in those directions. A mighty palace is to the east. There is a board here with the current news on it.");
...
int cmd_start(string str)
{
    ...
    this_player()->set_primary_start("/d/shadow/rooms/tene10.c");
