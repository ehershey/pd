...
string query_phase_name(int phase) {
    if (phase == 0 ) return "Error, phase is Null";
...
void la_sky(string str) {
    ...
    if(environment(this_player())->query_property("indoors")) {
      message("no_wrap", DEFAULT_MSG, this_player());
      return;
    }
    ...
    switch(str) {
        ...
        case "sky":
          ...
          break;
        case "moons":
          if(tod != "night") message("no_wrap", DEFAULT_MSG, this_player());
          else {
            if(phase[0] != "new") write("There is a "+MOON_DESC[0]+".");
            if(phase[1] != "new") write("There is a "+MOON_DESC[1]+".");
            if(phase[2] != "new") write("There is a "+MOON_DESC[2]+".");
              if ((phase[0] == "new")&&(phase[1] == "new")&&
                  (phase[2] == "new") ) message("no_wrap", DEFAULT_MSG, this_player());
          }
          break;

        case "laros": case "red moon":
          if(tod != "night" || phase[0] == "new") message("no_wrap", DEFAULT_MSG, this_player());
          else {
        ...
        case "slayar": case "green moon":
          if(tod != "night" || phase[2] == "new") message("no_wrap", DEFAULT_MSG, this_player());
