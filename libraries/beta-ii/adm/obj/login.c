...
//  Get a random file from /news/login/
    ...
    message("logon", center("Copyright (c) 1999-2010 Primal Darkness Productions."), this_object());
    if (!i)
...
static void get_name(string str) { 
    ...
    if(!user_exists(__Name)) { 
        ...
        message("logon", terminal_colour(
            header_line("New Character")+"\n"+
            "%^CYAN%^"+
            "Welcome to Primal Darkness.\n\n"
            "You are on the first step to entering this world.\n\n"
            "The name you've chosen is "
            "%^YELLOW%^"+capitalize(__Name)+"%^RESET%^%^CYAN%^.\n"
            "We want everyone's time here to be as enjoyable as possible,\n"
            "so please read the following guidelines for character names.\n"+
            header_line()+"\n\n"+
            "Press enter to continue: ",
            TERMINAL_D->query_term_info("ansi-status"), 80, 0), 
          this_object()); 
        input_to("name_policy"); 
        return; 
    } 
...
static void name_policy(string str) {
  message("logon", read_file(NAME_POLICY), this_object());
  message("logon", "\n\n"
    "Do you want to keep the name "+capitalize(__Name)+"? [(y)es/(n)o]: ",
    this_object() );
  input_to("new_user");
}
