...
message("logon", "<img src=\"http://www.iuis.org/mud/pueblo/greendragon.gif\">", this_object());
 ...
 message("logon", center("Copyright (c) 1999-2003 Primal Darkness Productions."), this_object());
    if (!i)
...
this_object());
   ...
   catch(CHAT_D->do_raw_chat("system", "System <system> Inbound connection from IP: "+query_ip_number(this_object())));
...
if (BANISH_D->query_total_ban(query_ip_number()))
  {
    message("logon", "\n---------------------------[ Site Banished ]---------------------------\n"
                     ...
                     " not create new characters, no play old ones.  We are sorry, but some\n"
                     " people do not get the hint to leave.  If you feel this is in error, \n"
                     " please mail us at stormbringer@primaldarkness.com\n"
                     "-----------------------------------------------------------------------\n", 
                     this_object());
    internal_remove();
    return;
