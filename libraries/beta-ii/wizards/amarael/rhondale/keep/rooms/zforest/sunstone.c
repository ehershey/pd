...
int readin(string str) {
	...
	message("read", "\n\n%^YELLOW%^ < .nuS eth sa ti ekame ot ratla eht no ti ecalP - .eno lacirehps tcefrep eht rof senots eht hcraeS >%^RESET%^\n", this_player(), );
...
int placin(string str) {
    ...
    message("info", this_player()->query_cap_name()+" places a sperical stone on the altar.", this_object(), this_player());
    call_out("done", 3, tp);
    return 1;
}
...
int searchin(string str) {
    ...
    if(present("worthmark", this_player()) || present("spherestone", this_player()) || present("sunstone", this_player()) || member_array("kratoan", this_player()->query_quests()) != -1 || this_player()->query_property("killersmark") == 1) return notify_fail("Why bother? You already found what you need.");
    if(searchit == 0) {
