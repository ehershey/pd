...
void create() {
 ::create();
   ...
   set_exits( ([ 
        "northwest" :  ROOMS "p_15",
        "northeast" :  ROOMS "p_16",
        "southwest" :  ROOMS "p_23",
        "down" :  ROOMS "dp_9",
        "southeast" : ROOMS  "p_25"]));
set_invis_exits(({"down"}));
    ...
    if(!present("mosquito 3")) new(MOB"mosq1.c")->move(this_object());
    if(!present("mosquito 4")) new(MOB"mosq2.c")->move(this_object());
}
