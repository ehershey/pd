...
void create() {
 ::create();
   ...
   set_exits( ([ 
        "northwest" :  ROOMS "tower4_3",
        "southwest" :  ROOMS "tower4_1"]));
}
void reset() {
::reset();
    if(!present("spider")) new(MOB"12spider.c")->move(this_object());
    if(!present("spider 2")) new(MOB"12spider.c")->move(this_object());
}
