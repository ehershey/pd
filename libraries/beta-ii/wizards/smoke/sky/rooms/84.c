...
void create() {
  ...
  set_exits( ([ 
    "northwest" :  ROOMS "83",
    "southeast" :  ROOMS "85"]));
  if(!present("dragon")) new(MOB"f_white2.c")->move(this_object());
  if(!present("dragon 2")) new(MOB"f_white2.c")->move(this_object());
  if(!present("dragon 3")) new(MOB"m_black2.c")->move(this_object());
  if(!present("dragon 4")) new(MOB"f_blue2.c")->move(this_object());
}
