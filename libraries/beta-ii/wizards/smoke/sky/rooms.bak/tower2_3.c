...
void create() {
 ::create();
   ...
   set_exits( ([ 
        "southwest" :  ROOMS "tower2_4",
        "southeast" :  ROOMS "tower2_2"]));
}
void reset() {
::reset();
    if(!present("leprechaun")) new(MOB"leprechaun.c")->move(this_object());
    if(!present("sprite")) new(MOB"sprite.c")->move(this_object());
}
