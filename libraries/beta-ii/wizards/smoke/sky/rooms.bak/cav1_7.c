...
void create() {
 ::create();
   ...
   set_exits( ([ 
	"northwest" : ROOMS "cav1_6",
	"south" : ROOMS "cav1_26",
	"southeast" : ROOMS "cav1_8",
	"southwest" : ROOMS "cav1_25"]));
}
void reset() {
::reset();
    if(!present("deathknight")) new(MOB"dk2.c")->move(this_object());
    if(!present("imp")) new(MOB"imp1.c")->move(this_object());
    if(!present("imp 2")) new(MOB"imp1.c")->move(this_object());
}
