...
void create() {
    ...
    set("long",
          "Inside the statue lies the Monk Hall.  All the walls are lined "
        "with gold except for the north wall which bears the golden crest "
        "of the Monk embedded in granite.  In front of the crest, sits a "
        "golden alter lined with rubies and diamonds.  Along the west wall "
        "are bookshelves stuffed with scrolls and tattered parchment which "
        "spills onto the floor.  There are stairs leading up at the south wall.");
    set_exits(
              (["out" : FORT "fort12"]) );
}
void init() {
...
string get_new_title(object tp)
{
    ...
    if(this_player()->query_guild()) {
        if(present((string)tp->query_guild()+"_ob", tp)) {
            title = (string)present((string)tp->query_guild()+"_ob",
tp)->query_title(tp);
            title += " $N";
        }
        else title = "$N";
...
way.\n");
    ...
    this_player()->set_stats(str, (int)this_player()->query_base_stats(str) +
1)
;
    ...
    else if(str == "intelligence") tell_room(this_object(), "more
intelligent.\n
");
...
int get_cost(string stat, int lev) {
    switch(stat) {
        case "intelligence": return (int)ADVANCE_D->get_stat_cost(1, lev);
break
;
