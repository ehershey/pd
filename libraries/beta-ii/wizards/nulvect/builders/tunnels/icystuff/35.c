...
set_long("The icy tunnels slip through many directions, sliding to nowhere. Large boulders of %^BOLD%^%^BLUE%^ice%^RESET%^ are being formed by the constant dripping of the ceiling. There is a strong sense of cleanliness here. The ground is covered in %^BOLD%^frozen%^RESET%^ stones of different sizes, scattered randomly throughout the %^BOLD%^snow%^RESET%^. The air is composed of a fresh mixture of %^BOLD%^cold%^RESET%^ and clean.");
set_items(([
...
set_exits(([ "northwest" : ROOMS "/icystuff/2.c"]));
}
