...
set_long("The icy tunnels slip through many directions, sliding to nowhere. Flakes of %^BOLD%^snow%^RESET%^ float through the air, sticking to any dry surfaces. The ground is covered in %^BOLD%^frozen%^RESET%^ stones of different sizes, scattered randomly throughout the %^BOLD%^snow%^RESET%^. Large boulders of %^BOLD%^%^BLUE%^ice%^RESET%^ are being formed by the constant dripping of the ceiling. A large %^BOLD%^i%^RESET%^%^BLUE%^c%^RESET%^%^BOLD%^i%^CYAN%^c%^RESET%^%^BOLD%^l%^RESET%^e falls from the ceiling, causing a crash that carries throughout the tunnels.");
set_items(([
...
set_exits(([ "west" : ROOMS "/icystuff/2.c",
"east" : ROOMS "/icystuff/4.c"]));
}
