...
set_long("The molten tunnels are %^RED%^%^BOLD%^flaming%^%^RESET%^ hot, and hard to see through with all the light. Stalagmite and stalactite make the tunnel look as if it is a mouth filled with fangs. The bubbling of %^RED%^l%^BOLD%^a%^YELLOW%^v%^RED%^a%^RESET%^ echoes off the walls. %^BOLD%^%^WHITE%^Bones%^RESET%^ float throughout the %^RED%^l%^BOLD%^a%^ORANGE%^%^RESET%^v%^YELLOW%^a%^BOLD%^%^RESET%^... what sort of being would be so strong to have indestructible bones? Shadows seem to creep through the smokey haze, although nothing can be seen for sure.");
set_items(([
...
set_exits(([ "northwest" : ROOMS "/moltenlava/4.c",
"east" : ROOMS "/moltenlava/6.c"]));
}
