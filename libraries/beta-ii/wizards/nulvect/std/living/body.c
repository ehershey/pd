...
void augment_body(int x) {
 ...
 query_max_hp()/((int)RACE_D->query_max_mon_dam(limbs[i],b_type)*5);
      return;
    } else
    for(i=0, tmp = sizeof(limbs); i<tmp; i++)
      {
      ...
      body[limbs[i]]["max_dam"] =
       (query_max_hp()/RACE_D->query_max_dam(limbs[i], this_object()->query_race()))*5;
...
string *query_walking_limbs() {
  ...
  if (!strlen(r = to->query_race()))
    if (!strlen(r = to->query_body_type()))
      return ({});
...
int query_mobile() {
  ...
  string *sev = filter(walk, (: $(to)->query_limb_status($1) & LIMB_SEVERED :) );
  if (sizeof(sev))
    return to_float(sizeof(sev)) < to_float(sizeof(walk)) * 0.4;
  return 1;
}
...
int do_damage(string limb, int damage) {
    // reduce damage for players
      if (this_object()->is_player()) damage = damage/2;

    // AC damage reduction
      damage = damage - this_object()->query_current_protection(limb);
      if(damage <= 0)
        return 0;
    ...
    if(body[limb]) {
        if(userp(this_object()))
            ...
            body[limb]["damage"] += damage/(this_object()->query_level() * 3);
...
int do_direct_damage(string limb, int damage) {
    //halve the pvp damage
      if(this_player()->is_player() && this_object()->is_player()) damage = damage/2;
...
void remove_kill(int i) { player_data["kills"] -= i; }
