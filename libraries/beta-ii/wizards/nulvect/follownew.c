...
int follow(object room, string dir) {
  ...
  if (tp && !tp->is_linkdead() && to->query_property("teleport follow")) {
    to->move_player(environment(tp), "following "+tp->query_cap_name());
    if (environment(to) != environment(tp)) to->move(environment(tp));
    ret = 1;
  }
