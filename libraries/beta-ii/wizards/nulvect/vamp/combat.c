...
int kill_ob(object victim, int which) {
  ...
  att = (object *)victim->query_attackers() || ({});
  if(!sizeof(att) || member_array(this_object(), att) == -1) {
    if(!ok_to_kill(victim)) {
      message("my_combat", "Supernatural forces prevent you "
	"from attacking "+victim->query_cap_name()+".",
       this_object());
      return 0;
    }
  }
...
int ok_to_kill(object targ) {
  ...
  if(targ->query_property("no attack")) return 0;
