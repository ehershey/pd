...
void heart_beat() {
  ...
  if (to->query_property("teleport follow"))
...
int pet_command(string str) {
  ...
  if (spell && member_array(cmd, to->query_spells()) == -1)
    ok = 0;

  if (ok)
    command(cmd+(arg?" "+arg:""));

  // this will make this fall through and not say What?
  if (all) return notify_fail("");
