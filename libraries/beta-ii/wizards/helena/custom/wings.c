...
int reveal(string str){
    ...
    if (names == ""){
      names = "%^CYAN%^" + str; save_object(dir); write("%^BOLD%^%^CYAN%^You reveal the name of your first sinner, "+str+"%^CYAN%^. A bright light glows from your wings and a sigil reveals itself.%^RESET%^");
      ...
      extralook = "%^BOLD%^%^BLACK%^Only one of the symbols may be recognised: "+names+".";
      set_long(desc);
      return 1;
}
    else {
...
int conceal(){ write("%^BOLD%^%^BLACK%^You conceal your %^CYAN%^sigils%^BLACK%^ once more.");
