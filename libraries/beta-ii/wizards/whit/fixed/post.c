...
_mail(0);
    ...
    switch(cmd) {
        ...
        case "g": do_groups(arg); break;
        case "w": set_forward(arg); break;
        case "?": do_help(arg); break;
        case "s": case "S": 
          if(!wizardp(this_player())) 
           message("mail", "Save option only for creators.\n",this_player());
            else do_save(cmd, arg);
            break;
        case "R": case "r": do_reply(cmd, arg); break;
        case "F": case "f": do_forward(cmd, arg); break;
        case "Q": case "q": do_quit(cmd); return;
        case "m": get_to(arg); break;
        case "d": case "u": handle___Delete(cmd, arg); break;
        case "h": do_header(arg); break;
        default:
...
void do_groups(string str) {
    ...
    if(!str || str == "") {
      message("system", "Personal and "+mud_name()+" groups for the IIPS:",
this_player());
...
"
        ...
        case "m":
          ...
          break;
        default:
       this_player()->more(explode(read_file(POSTAL_USER_HELP), "\n"), "help", 
            (: do_mail(0) :));
          return;
    }
    do_mail(0);
...
void get_text() {
    if(__FwdFlag != -1) {
        write_file(DIR_TMP+"/"+__TmpPost["from"]+".iips", "\n-----\n"+
          ...
          " to "+capitalize(__Owner)+" on
"+ctime(__BoxInfo[__FwdFlag]["date"])+
         
":\n-----\n"+(string)LETTER_D->query_letter(__BoxInfo[__FwdFlag]["id"]));
        __FwdFlag = -1;
    }
    __TmpPost["message"] =
read_file(DIR_TMP+"/"+__TmpPost["from"]+".iips")+"\n";
...
void do_reply(string cmd, string arg) {
    ...
    if(cmd == "R") {
        tmp =
explode((string)LETTER_D->query_letter(__BoxInfo[__Current]["id"]),
          "\n");
...
"+capitalize(__BoxInfo[__Current]["from"])+" to "+capitalize(__Owner)+"
"+ctime(__BoxInfo[__Current]["date"])+":\n"+
          "-----\n");
        write_file(DIR_TMP+"/"+__TmpPost["from"]+".iips",
          (string)LETTER_D->query_letter(__BoxInfo[__Current]["id"]));
        get_text();
...
string letter_head(mapping this_letter) {
    ...
    ret += ""+capitalize(this_letter["from"])+"\n";
    ret += "To: ";
    ...
    ret += "Subject: "+this_letter["subject"]+"\n";
    return ret;
}
