...
int markamon(string str) {
    ...
    if(strsrch(str, " ") != -1) {
	write("You can only mark a one word place.\n");
    }
    env = environment(tp);
    if (env->query_property("no teleport")) { 
	write("You cannot make a seal of this spot.\n"); 
	return 1;
    }
    envname = str;
...
int breakingz0r(string str) {
	...
	write("You cannot break a seal here.\n"); 
	return 1;
    }
    if (this_player()->query_current_attacker()) { 
	write("You cannot do this in battle!\n"); 
	return 1;
    }
    if (this_player()->query_arena()) { 
	write("You cannot do this in the arena!"); 
	return 1;
    }
    env = find_object(envpath);
    write("%^BOLD%^%^GREEN%^Mystical energies engulf you!\n%^RESET%^");
    tell_room(environment(tp), "%^BOLD%^%^GREEN%^Mystical energies engulf "+
...
int goingalready(object tp) {
    ...
    message("info", "%^BOLD%^%^GREEN%^"+tp->query_cap_name()+" vanishes from "
      "sight!%^RESET%^", environment(tp), tp);
    tp->move_player(env);
    this_object()->remove();
    return 1;
}
